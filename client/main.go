...
type metricPayload struct {
	...
	// Dedicated-server hardware snapshot (Linux only, optional).
...
func main() {
	...
	// CLIENT_PORT=0 disables the probe's own HTTP listener: push mode needs
	// none, and a dedicated server should not expose an extra port. CLIENT_BIND
	// narrows the bind address (e.g. 127.0.0.1) when the listener is kept.
	listenerEnabled := clientPort != "0" && clientPort != ""
	clientBind := envOr("CLIENT_BIND", "")
	secret = envOr("SECRET", "")

	// Record start time for uptime calculation
	startTime = time.Now()

	// Dedicated-server hardware collector (Linux); other platforms are no-ops.
	startHardwareCollector()
	...
	addr := clientBind + ":" + clientPort
	var server *http.Server
	serverErrCh := make(chan error, 1)
	if listenerEnabled {
		server = startClientListener(addr, mux, serverErrCh)
	} else {
		log.Printf("📡 HTTP listener disabled (CLIENT_PORT=0): push mode only, no port exposed")
	}
...
// startClientListener serves /metrics, /health and /tcping for pull mode.
// The listener carries TCP keepalive (60 s) so server→client connections are
// not dropped by Windows firewalls / NAT.
func startClientListener(addr string, mux *http.ServeMux, serverErrCh chan error) *http.Server {
...
// registrationPort is the port the server may poll for pull mode; empty when
// the listener is disabled so the server never builds a poll URL.
func registrationPort() string {
	p := envOr("CLIENT_PORT", "9090")
	if p == "0" {
		return ""
	}
	return p
	...
	for range ticker.C {
		...
		if resp.StatusCode == http.StatusOK {
			if metrics.Hardware != nil {
				markHardwarePushed(metrics.Hardware.CollectedAt)
			}
...
func getCPUModel() string {
	...
	if runtime.GOOS == "windows" || runtime.GOOS == "darwin" {
		// Topology: the hardware collector's WMI / sysctl figures are
		// authoritative. Until its first snapshot exists (a few seconds after
		// start) fall back to gopsutil and skip the cache, so an SMT machine
		// ends up as "4 Core / 8 Thread" and never keeps a stale
		// "8 Physical Core".
		physical, logical, known := 0, 0, false
		hwMu.RLock()
		if hwLatest != nil && hwLatest.CPU != nil && hwLatest.CPU.Cores > 0 && hwLatest.CPU.Threads > 0 {
			physical, logical, known = hwLatest.CPU.Cores, hwLatest.CPU.Threads, true
		}
		hwMu.RUnlock()
		if physical == 0 {
		...
		coreLabel := fmt.Sprintf("%d %s Core", coreCount, coreType)
		if coreType == "Physical" && physical > 0 && logical > physical {
			coreLabel = fmt.Sprintf("%d Core / %d Thread", physical, logical)
		}
		if speed != "" && coreCount > 0 {
			result = fmt.Sprintf("%s @ %sGHz %s", model, speed, coreLabel)
		} else if coreCount > 0 {
			result = fmt.Sprintf("%s %s", model, coreLabel)
		} else {
			result = model
		}
		if known {
	...
	// Dedicated servers: report the real topology ("4 Core / 8 Thread")
	// instead of labelling every logical CPU a "Physical Core".
	coreSuffix := fmt.Sprintf("%d %s Core", coreCount, coreType)
	if coreType == "Physical" && physicalCores > 0 && virtualCores > physicalCores {
		coreSuffix = fmt.Sprintf("%d Core / %d Thread", physicalCores, virtualCores)
	}
	var final string
	if speedLinux != "" && coreCount > 0 {
		final = fmt.Sprintf("%s @ %sGHz %s", model, speedLinux, coreSuffix)
	} else if coreCount > 0 {
		final = fmt.Sprintf("%s %s", model, coreSuffix)
	} else {
		final = model
	}
	cacheMutex.Lock()
